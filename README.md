...
- **Código não encontrado?** Dá pra vincular na hora a um produto já existente (busca
  por nome) sem parar a contagem — o app já reconhece esse código nos próximos scans do
  aparelho, e o vínculo fica registrado na aba **Códigos novos** pra você revisar depois
  e incluir no `dados.json`.
- **Etiqueta nova:** no card do produto dá pra marcar "precisa de etiqueta nova" (com ou
  sem quantidade preenchida) — a linha fica destacada em âmbar na aba Contagens.
...
| **🏷️ Ver produtos p/ etiqueta nova**               | Lista todos os produtos marcados no app como "precisa de etiqueta nova" (linha, código e nome).                                                                                                                                                                                                                                                                                                                                      |
| **🏷️ Limpar marcações de etiqueta (já imprimi)**   | Zera a coluna Etiqueta nova de todo mundo — roda depois de imprimir as etiquetas pendentes.                                                                                                                                                                                                                                                                                                                                          |
...
`Letra · Código · Produto · Qtd loja · Qtd estoque · Total · Estoque sistema · Ajuste · Status · Atualizado em · Etiqueta nova`

- **Ajuste** (automático): `Aumentar N` / `Diminuir N` / `Conferir (0 no sist.)` / `—`.
- **Status**: `Não contado` · `OK` · `Pendente` · `Feito` · `Inativado` · `Enviado`.
- **Etiqueta nova**: `Sim` / vazio — marcado no card do produto, no app. Linha fica em
  âmbar. Menu **🏷️ Ver produtos p/ etiqueta nova** lista tudo; **🏷️ Limpar marcações**
  zera depois de imprimir.

## Aba Códigos novos

Só de revisão — não afeta a contagem. Toda vez que alguém vincula, no app, um código de
barras não cadastrado a um produto existente (em vez de parar a contagem pra ir cadastrar),
entra uma linha aqui: `Data · Código escaneado · Produto vinculado · Chave produto · Letra`.
De tempos em tempos, olhe essa aba e inclua os códigos novos no `dados.json` (a skill de
atualização de estoque faz isso a partir de uma lista de códigos). O app também lembra o
vínculo localmente no aparelho — reconhece o código nos próximos scans mesmo antes de você
atualizar a base.
...
`{"ok":true,"servico":"contagem-estoque-lc","versao":9.0}`.
...
- **Segurança:** o `SYNC_TOKEN` no site é só anti-acesso-casual. O endpoint público do
  Web App **só grava nas abas Contagens e Códigos novos** (v9) — não alcança a
  Estoque_Principal nem o resto do Drive. As ações que tocam a Estoque_Principal ("Fechar
  letra", "Atualizar estoque") rodam **só pelo menu**, nunca pela URL pública. Redeployar
  invalida a URL antiga.

---

## Stack
HTML + CSS + **React 18** (UMD) · **html5-qrcode** · **Google Apps Script** + **Google Sheets** · **GitHub Pages**.# Contagem de Estoque — Loja da Construção
